...
- `data/contracts.json` : 계약업무 처리지침 원문 데이터(서버 함수에서 사용)
- `vercel.json` : Vercel 설정

## 가장 쉬운 배포 방법

1. 이 ZIP을 풀어 GitHub 새 저장소에 모든 파일을 업로드합니다.
2. Vercel에서 `Add New > Project`를 누릅니다.
3. 방금 만든 GitHub 저장소를 Import 합니다.
4. 프로젝트의 `Settings > Environment Variables`에서 다음 값을 추가합니다.
   - `GEMINI_API_KEY` = 본인의 Gemini API 키
   - `GEMINI_MODEL` = `gemini-3.7-flash` (선택 사항)
5. `Deploy` 또는 `Redeploy` 합니다.

API 키는 Vercel 서버 환경변수에서만 사용되고 `index.html`에는 들어가지 않습니다.

## 주의

- 공개 주소로 배포하면 API 키 자체는 노출되지 않지만 누구나 챗봇을 호출해 API 비용을 발생시킬 수 있습니다.
- 현재 동일 IP 기준 10분당 25회 기본 제한을 넣었습니다. 서버리스 인스턴스 메모리 기반이므로 강력한 비용 통제가 필요하면 Vercel KV/Upstash 기반 제한이나 로그인 기능을 추가하는 것이 좋습니다.
